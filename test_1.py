...
# Obtain joint limits for the Franka Panda robot
joint_limits = [p.getJointInfo(panda, i)[8:10] for i in range(p.getNumJoints(panda))]
...
def robot_extend_fn(config1, config2):
    ...
    # Generate the path
    path = [config1 + i * 0.01 * diff for i in range(int(np.linalg.norm(diff)/0.01))]
...
# Ensure that the desired pose is reachable by the robot
if p.calculateInverseKinematics(panda, 11, burg.util.position_and_quaternion_from_tf(desired_pose, convention='pybullet')[0]) is None:
    raise ValueError("Desired pose is not reachable by the robot.")
...
try:
    ...
    print("Error in path planning:", e)
    path = []
